// WebXR AR 控制器：6DoF 位置 + 旋转追踪，但隐藏相机画面
//
// 关键：Three.js r169 的 WebXRManager 在 AR 模式下强制 alpha:true（透明），
// 会显示相机透传。我们在 session 创建后手动替换为 alpha:false 的 XRWebGLLayer，
// 让 framebuffer 不透明，从而只显示 3D 场景，不显示相机画面。
//
// 这样实现了与 iOS ARKit .nonAR 模式一致的体验：
// - 6DoF 追踪（走路时位置变化反映到虚拟相机）
// - 不显示相机画面（只有 3D 场景）
...
export class WebXRARController {
  ...
  /** 进入 AR 会话，初始化 referenceSpace，记录初始姿态，并替换为不透明 baseLayer */
  async enter(renderer: THREE.WebGLRenderer, initPose: THREE.Matrix4): Promise<void> {
    ...
    // 关键：Three.js r169 在 AR 模式下会用 alpha:true 创建 baseLayer（显示相机画面）。
    // 我们手动替换为 alpha:false 的 baseLayer，让 framebuffer 不透明，隐藏相机画面。
    // 这样只显示 3D 场景，与 iOS .nonAR 模式体验一致。
    const gl = renderer.getContext();
    const opaqueLayer = new XRWebGLLayer(session, gl, {
      alpha: false,
      antialias: true,
      depth: true,
      stencil: false,
    });
    session.updateRenderState({ baseLayer: opaqueLayer });

    // 确保渲染器清屏颜色不透明（与 scene.background 一致）
  ...
  private applyDeviceMotion(deviceTransform: THREE.Matrix4, camera: THREE.PerspectiveCamera): void {
    ...
    //    真实世界走动 → 设备 position 变化 → 经 yaw 对齐叠加到 edit 平移
